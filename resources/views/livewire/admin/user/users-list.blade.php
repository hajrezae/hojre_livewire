<x-admin.card.simple title="لیست کاربران" icon="fas fa-user">
    ...
    <x-slot name="body">
        ...
        <div wire:loading.class="overlay overlay-block">
                    ...
                    <x-slot name="body">
                        @forelse($users as $user)
                            <tr>
                                ...
                                <td scope="col" class="align-middle">
                                    <div class="d-flex justify-content-start align-items-center">
                                        <img
                                                src="{{ $user->avatar_url != null ? asset($user->avatar_url) : asset('media/users/default.jpg') }}"
                                                class="rounded-lg" width="50px" height="50px" alt="user avatar"
                                        >
                                ...
                                <td
                                        scope="col" class="align-middle text-muted font-weight-boldest"
                                >{{ $user->email }}</td>
                                <td scope="col" class="align-middle">{{ $user->first_name }}</td>
                                <td scope="col" class="align-middle">{{ number_format($user->wallet) }}
                                    <span class="currency text-success font-weight-boldest">{{ \App\Services\Shop::currency()  }}</span>
                                </td>
                            ...
                            <span class="d-flex">
                                ...
                                ><i class="flaticon2-edit"></i></a>
                                <a
                                        href="{{ route('admin.user.address.index', $user) }}"
                                        class="btn btn-sm btn-icon btn-light-primary mr-2"
                                ><i class="flaticon2-location"></i></a>
                                <a class="btn btn-sm btn-icon btn-light-danger mr-2"><i
                                            class="flaticon-delete-1"
                                            onclick="confirmDelete(event, {{ $user->id }}, 'کاربر', 'deleteUser')"
                                    ></i></a>
                            </span>
                                </td>
                            </tr>
            ...
            <div wire:loading.class.remove="d-none" class="overlay-layer rounded bg-light-o-80 d-none">
